...
const EnterLoading = () => {
  ...
  const columnVariants = {
    ...
    animate: { y: "100%", opacity: 1, transition: { duration: 0.3, ease: "easeIn" } },
  };
  ...
  return (
    <AnimatePresence mode="wait">
      {isLoading && (
        <motion.div
          className="inset-0 fixed z-50 grid grid-cols-4 overflow-hidden"
        ...
        >
          {data.map((item, index) => (
            <motion.div
              key={index}
              className="bg-yellow-300 flex items-center justify-center overflow-hidden"
